...
Rank: 10 (3.5% of GPU time)
Model shape: batch=1, heads=12, seq_len=512, head_dim=64
...
MODEL_SHAPES = {'batch': 1, 'heads': 12, 'seq_len': 512, 'head_dim': 64}

# Benchmark config (self-describing -- bench.py can load this dynamically)
TEST_SIZES = [
    ("model_primary", {'batch': 1, 'heads': 12, 'seq_len': 512, 'head_dim': 64}),
    # Also test nearby sizes for robustness
    ("model_half", {'batch': 1, 'heads': 12, 'seq_len': 256, 'head_dim': 64}),
    ("model_double", {'batch': 2, 'heads': 12, 'seq_len': 512, 'head_dim': 64}),
]
...
def flash_attention_kernel(
    ...
    for start_n in range(0, kv_end, BLOCK_N):
        ...
        # fp16 QK dot + sm_scale with LOG2E baked in
