...
Op type: gemm
Rank: 11 (4.0% of GPU time)
Model shape: M=1024, N=768, K=3072
...
MODEL_SHAPES = {'M': 1024, 'N': 768, 'K': 3072}

# Benchmark config (self-describing -- bench.py can load this dynamically)
TEST_SIZES = [
    ("model_primary", {'M': 1024, 'N': 768, 'K': 3072}),
    # Also test nearby sizes for robustness
    ("model_half", {'M': 512, 'N': 384, 'K': 1536}),
    ("model_double", {'M': 2048, 'N': 1536, 'K': 6144}),
]
...
def kernel_fn(A: torch.Tensor, B: torch.Tensor) -> torch.Tensor:
    ...
    # On Orin SM87: Triton fp16/bf16/fp32 accumulation diverges from cuBLAS for large K.
    # cuBLAS is also faster for this compute-bound shape. Use torch.matmul for all dtypes.
