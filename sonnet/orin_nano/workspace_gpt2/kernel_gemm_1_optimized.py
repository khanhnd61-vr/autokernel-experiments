...
Op type: gemm
Rank: 1 (16.1% of GPU time)
Model shape: M=1024, N=50257, K=768
...
TEST_SIZES = [
    ...
    ("model_half", {'M': 512, 'N': 25128, 'K': 384}),
    ("model_double", {'M': 2048, 'N': 100514, 'K': 1536}),
]
...
def kernel_fn(A: torch.Tensor, B: torch.Tensor) -> torch.Tensor:
    ...
    N = B.shape[1]
    # Use Triton only for very large-N (vocab-scale: N≥10000) float16 GEMMs.
    # For smaller N (attention/MLP: N=768..3072), cuBLAS is faster on Orin SM87.
    # For non-float16, cuBLAS has better accuracy (bfloat16/float32 1-ULP issue).
    if A.dtype != torch.float16 or N < 10000:
