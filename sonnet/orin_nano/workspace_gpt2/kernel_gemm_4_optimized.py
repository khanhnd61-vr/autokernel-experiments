...
Op type: gemm
Rank: 4 (7.5% of GPU time)
Model shape: batch=12, M=1024, N=1024, K=64
...
MODEL_SHAPES = {'batch': 12, 'M': 1024, 'N': 1024, 'K': 64}

# Benchmark config (self-describing -- bench.py can load this dynamically)
TEST_SIZES = [
    ("model_primary", {'batch': 12, 'M': 1024, 'N': 1024, 'K': 64}),
    # Also test nearby sizes for robustness
    ("model_half", {'batch': 6, 'M': 512, 'N': 512, 'K': 32}),
    ("model_double", {'batch': 24, 'M': 2048, 'N': 2048, 'K': 128}),
]
...
def kernel_fn(A: torch.Tensor, B: torch.Tensor) -> torch.Tensor:
    ...
    K = A.shape[1]
    # On Orin SM87: large-K accumulation diverges from cuBLAS beyond tolerance for float16,
    # and bfloat16/float32 always diverge. Use Triton only for the model-specific K=64 shape
    # where a single BLOCK_SIZE_K=64 iteration has zero accumulation error.
    if A.dtype != torch.float16 or K > BLOCK_SIZE_K:
