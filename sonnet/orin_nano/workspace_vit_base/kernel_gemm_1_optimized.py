...
Op type: gemm
Rank: 1 (17.3% of GPU time)
Model shape: M=197, N=3072, K=768
...
MODEL_SHAPES = {'M': 197, 'N': 3072, 'K': 768}

# Benchmark config (self-describing -- bench.py can load this dynamically)
TEST_SIZES = [
    ("model_primary", {'M': 197, 'N': 3072, 'K': 768}),
    # Also test nearby sizes for robustness
    ("model_half", {'M': 98, 'N': 1536, 'K': 384}),
    ("model_double", {'M': 394, 'N': 6144, 'K': 1536}),
]
...
def kernel_fn(A: torch.Tensor, B: torch.Tensor) -> torch.Tensor:
    ...
    # On Orin SM87: Triton summation order diverges from cuBLAS across all dtypes for
    # large shapes (K>1024). cuBLAS is also faster for these small M=197 shapes.
