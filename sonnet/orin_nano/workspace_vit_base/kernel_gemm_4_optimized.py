...
Op type: gemm
Rank: 4 (8.8% of GPU time)
Model shape: batch=12, M=197, N=197, K=64
...
MODEL_SHAPES = {'batch': 12, 'M': 197, 'N': 197, 'K': 64}

# Benchmark config (self-describing -- bench.py can load this dynamically)
TEST_SIZES = [
    ("model_primary", {'batch': 12, 'M': 197, 'N': 197, 'K': 64}),
    # Also test nearby sizes for robustness
    ("model_half", {'batch': 6, 'M': 98, 'N': 98, 'K': 32}),
    ("model_double", {'batch': 24, 'M': 394, 'N': 394, 'K': 128}),
]
...
def kernel_fn(A: torch.Tensor, B: torch.Tensor) -> torch.Tensor:
    ...
    # On Orin SM87: Triton fp32 summation diverges from cuBLAS for large shapes.
    # Use Triton for small shapes (model-specific range, correctness confirmed),
    # dispatch large shapes to cuBLAS for correctness.
    M, K = A.shape
    _, N = B.shape
    if max(M, N, K) > 512:
