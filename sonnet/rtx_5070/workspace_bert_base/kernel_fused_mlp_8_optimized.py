...
Op type: fused_mlp
Rank: 8 (2.9% of GPU time)
Model shape: batch=4096, dim=768, hidden=3072
(BERT-base: batch*seq=8*512=4096, input_hidden=768, intermediate=3072 -- corrected from profiler parse error)
NOTE: fused_mlp kernel is SwiGLU (gate+up+silu); BERT uses GELU MLP. Shape is correct but
      the kernel computes gate*silu(up) rather than BERT's dense1+gelu. Use as a proxy for
      MLP throughput optimization at this shape.
...
# BERT-base MLP: batch*seq=4096 rows, input_hidden=768, intermediate=3072
MODEL_SHAPES = {'batch': 4096, 'dim': 768, 'hidden': 3072}

# Benchmark config (self-describing -- bench.py can load this dynamically)
TEST_SIZES = [
    ("model_primary", {'batch': 4096, 'dim': 768, 'hidden': 3072}),
    # Also test nearby sizes for robustness
    ("model_half", {'batch': 2048, 'dim': 384, 'hidden': 1536}),
    ("model_double", {'batch': 8192, 'dim': 1536, 'hidden': 6144}),
]
...
def triton_fused_gate_up_kernel(
    ...
    Uses L2 swizzle for better cache reuse.
    """
    pid = tl.program_id(0)
