...
Rank: 6 (4.4% of GPU time)
Model shape: batch=8, heads=12, seq_len=512, head_dim=64
(BERT-base: batch=8, num_heads=12, seq_len=512, head_dim=64 -- corrected from profiler parse error)
...
# BERT-base (8, 512): batch=8, num_heads=12, seq_len=512, head_dim=768//12=64
MODEL_SHAPES = {'batch': 8, 'heads': 12, 'seq_len': 512, 'head_dim': 64}

# Benchmark config (self-describing -- bench.py can load this dynamically)
TEST_SIZES = [
    ("model_primary", {'batch': 8, 'heads': 12, 'seq_len': 512, 'head_dim': 64}),
    # Also test nearby sizes for robustness
    ("model_half", {'batch': 4, 'heads': 12, 'seq_len': 256, 'head_dim': 64}),
    ("model_double", {'batch': 16, 'heads': 12, 'seq_len': 512, 'head_dim': 64}),
]
...
def triton_flash_attention_kernel(
    ...
    for start_n in range(0, kv_end, BLOCK_N):
        ...
        # Compute QK^T [BLOCK_M, BLOCK_N] using fp16 tensor cores
