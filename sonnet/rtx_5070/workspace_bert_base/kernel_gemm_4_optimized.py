...
Op type: gemm
Rank: 4 (13.3% of GPU time)
Model shape: M=4096, N=3072, K=768
(BERT-base: batch*seq=8*512=4096, hidden=768, intermediate=3072 -- MLP up-projection shape)
...
# BERT-base MLP up-projection: batch*seq=4096, hidden=768 -> intermediate=3072
MODEL_SHAPES = {'M': 4096, 'N': 3072, 'K': 768}

# Benchmark config (self-describing -- bench.py can load this dynamically)
TEST_SIZES = [
    ("model_primary", {'M': 4096, 'N': 3072, 'K': 768}),
    # Also test nearby sizes for robustness
    ("model_half", {'M': 2048, 'N': 1536, 'K': 384}),
    ("model_double", {'M': 8192, 'N': 6144, 'K': 1536}),
]
...
def triton_gemm_kernel(
    ...
    """
    Triton kernel with L2 swizzle for better cache reuse.
    """
    pid = tl.program_id(0)
